<template>
  <div>
    ...
    <el-table border ref="brtTable" v-loading="loading" :data="marketMaterielList">/>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" v-if="userId && (checkRole(['admin']) || userId == $store.state.user.id) && nodeStatus == '1'">
        <template slot-scope="scope">
          <el-button size="mini" type="text" @click="handleDelete(scope.row)"
            v-hasPermi="['order:marketMateriel:remove']">撤销</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  import {
    listMarketMateriel,
    listMarketMaterielAll,
    getMarketMateriel,
    delMarketMateriel,
    addMarketMateriel,
    updateMarketMateriel
  } from "@/api/order/marketMateriel";
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        listMarketMateriel(this.queryParams).then(response => {
          this.marketMaterielList = response.rows;
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('确认撤销？').then(function() {
          return delMarketMateriel(recordIds);
        }).then(() => {
          this.getList();
