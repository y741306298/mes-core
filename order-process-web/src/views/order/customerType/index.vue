  ...
  <div>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="0px">
        ...
        <el-table border ref="brtTable" v-loading="loading" :data="customerTypeList">
  ...
  export default {
    ...
    methods: {
      ...
      handleEditSubmit(row) {
        updateCustomerType(row).then(response => {
      ...
      getList() {
        this.loading = true;
        listCustomerTypeAll().then(response => {
      ...
      /** 提交按钮 */
      submitForm() {
        addCustomerType(this.form).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('确认删除？').then(function() {
          return delCustomerType(typeIds);
        }).then(() => {
          this.getList();
          this.$emit('getList')
