      ...
      <div id="printAre">
        <div style="text-align: center;">
          ...
          <div class="statement-table-title">
            <div class="textl">采购总额:¥<span> {{otherData.totalAmount}}</span></div>
            <div>
        ...
        <table id="table" style="width: 100%;border-collapse: collapse;border-spacing: 0;text-align: center;" border="1">
          ...
          <tbody>
            <tr v-for="(item,index) in marketOrderList">
              <td>{{parseTime(item.marketOrderVo.orderTime,'{y}-{m}-{d}')}}</td>
              <td>{{item.supplierName}}</td>
              <td>{{item.materielVo?item.materielVo.materielName:item.materielId}}</td>
              <td>{{item.marketOrderVo.totalNum}}</td>
              <td>{{item.marketOrderVo.totalAmount}}</td>
