    ...
    <el-dialog title="打印" :visible.sync="open" width="70%">
      <div id="printAre">
        ...
        <table style="width: 100%;border-collapse: collapse;border-spacing: 0;text-align: center;" border="1">
          ...
          <tbody>
            <tr v-for="(item,index) in salesOrderDetailsList">
              <td>{{index + 1}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       * 获取订单详情列表
       */
      getSalesOrderDetailsList(orderId) {
        const query = {
          orderId: orderId
        };
        listSalesOrderDetailsAll(query).then(res => {
          this.salesOrderDetailsList = res.data;
        })
      },
      /**
       * 拼接客户地址
      ...
      handleOpen(orderId) {
        ...
        this.getSalesOrderDetailsList(orderId);
      },
      /**
       * 获取订单详情
       */
      getOrderInfo(orderId) {
        this.reset();
        getSalesOrder(orderId).then(res => {
          this.form = res.data
        })
      },
      // 表单重置
