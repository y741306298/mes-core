      ...
      <div id="printAre">
        ...
        <el-descriptions title="">
          ...
          <el-descriptions-item label="供应商名称"
            v-if="form.supplierVo">{{form.supplierVo.supplierName}}</el-descriptions-item>
          <el-descriptions-item label="销售日期">{{parseTime(form.orderTime,'{y}-{m}-{d}')}}</el-descriptions-item>
          <el-descriptions-item label="收货日期">{{parseTime(form.deliveryTime,'{y}-{m}-{d}')}}</el-descriptions-item>
          <el-descriptions-item :span="2" label="收货地址"
            v-if="form.supplierAddressVo">{{spliceSupplierAddress(form.supplierAddressVo)}}</el-descriptions-item>
        ...
        <table style="width: 100%;border-collapse: collapse;border-spacing: 0;text-align: center;" border="1">
          ...
          <tbody>
            <tr v-for="(item,index) in marketOrderDetailsList">
              <td>{{index + 1}}</td>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       * 获取订单详情列表
       */
      getMarketOrderDetailsList(orderId) {
        const query = {
          orderId: orderId
        };
        listMarketOrderDetailsAll(query).then(res => {
          this.marketOrderDetailsList = res.data;
        })
      },
      /**
       * 拼接供应商地址
       */
      spliceSupplierAddress(supplier) {
        if(supplier.addressType == '0'){
          const address =
            `${supplier.addressShort}-${supplier.addressProvince}${supplier.addressCity}${supplier.addressArea}${supplier.addressDetails}`
          return address;
        }else{
          const address =
            `${supplier.destination}-${supplier.logisticsCompany}-${supplier.addressProvince}${supplier.addressCity}${supplier.addressArea}${supplier.addressDetails}`
          return address;
        }
      ...
      handleOpen(orderId) {
        ...
        this.getMarketOrderDetailsList(orderId);
      },
      /**
       * 获取订单详情
       */
      getOrderInfo(orderId) {
        this.reset();
        getMarketOrder(orderId).then(res => {
          this.form = res.data
        })
      },
      // 表单重置
