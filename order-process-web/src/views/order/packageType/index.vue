  ...
  <div>
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="0px">
        ...
        <el-table border ref="brtTable" v-loading="loading" :data="packageTypeList">
          <el-table-column label="封装类型" align="center">
            <template slot-scope="scope">
              <el-input v-if="scope.row.isEdit" v-model="scope.row.typeName" placeholder="请输入封装类型" />
  ...
  export default {
    ...
    methods: {
      ...
      handleEditSubmit(row) {
        updatePackageType(row).then(response => {
      ...
      getList() {
        this.loading = true;
        listPackageTypeAll().then(response => {
      ...
      /** 提交按钮 */
      submitForm() {
        addPackageType(this.form).then(response => {
