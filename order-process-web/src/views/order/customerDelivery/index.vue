<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-truck" size="mini"
          @click="handleDelivery">送货</el-button>
      </el-col>
      <right-toolbar :columns="columnList" :cacheKey="cacheKey" :showSearch.sync="showSearch"
        @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table border ref="brtTable" v-loading="loading" :data="customerDeliveryList"
...
<script>
  ...
  export default {
    ...
    dicts: ['delivery_status', 'collection_status'],
    data() {
      return {
        // 遮罩层
        loading: true,
    ...
    methods: {
      ...
      getList() {
        ...
        this.loading = true;
        queryMarketRecord(queryData).then(response => {
          this.customerDeliveryList = response.rows;
      ...
      handleUpdate(row) {
        ...
        getCustomerDelivery(deliveryId).then(response => {
      ...
      handleDelivery(){
        this.$router.push({
          path: '/customerDelivery/deliveryRemind/index',
          query: {
            customerId: this.customerId
          }
        });
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            if (this.form.deliveryId != null) {
              updateCustomerDelivery(this.form).then(response => {
                this.$modal.msgSuccess("修改成功");
                this.open = false;
                this.getList();
              });
            } else {
              addCustomerDelivery(this.form).then(response => {
      ...
      handleDelete(row) {
        ...
        this.$modal.confirm('确认删除？').then(function() {
          return delCustomerDelivery(deliveryIds);
        }).then(() => {
          this.getList();
      ...
      /** 导出按钮操作 */
      handleExport() {
        this.download('order/customerDelivery/export', {
          ...this.queryParams
        }, `customerDelivery_${new Date().getTime()}.xlsx`)
      }
    }
  };
</script>
