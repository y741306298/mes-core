  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="搜索" prop="keyWords">
        <el-input v-model="queryParams.keyWords" placeholder="请输入供应商名称/供应商编号" clearable
          @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="联系电话" prop="phone">
        <el-input v-model="queryParams.phone" placeholder="请输入联系电话" clearable @keyup.enter.native="handleQuery" />
      </el-form-item>
      <el-form-item label="供应商类型" prop="supplierType" label-width="120">
        <el-select v-model="queryParams.supplierType" clearable filterable class="drag-screenful-contnet">
          <el-option v-for="dict in dict.type.supplier_type" :key="dict.value" :value="dict.value"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
          v-hasPermi="['order:supplier:add']">新增供应商</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single" @click="handleUpdate"
          v-hasPermi="['order:supplier:edit']">编辑</el-button>
      </el-col>
      ...
      <el-col :span="1.5">
        <el-dropdown split-button type="primary" @command="handleDropdownClick" size="mini">
          ...
          <el-dropdown-menu slot="dropdown">
            ...
            <el-dropdown-item command="import" v-hasPermi="['order:supplier:import']">导入供应商</el-dropdown-item>
            <el-dropdown-item command="export" v-hasPermi="['order:supplier:export']">导出供应商</el-dropdown-item>
            <el-dropdown-item command="delete" :disabled="multiple"
              v-hasPermi="['order:supplier:remove']">删除</el-dropdown-item>
            <el-dropdown-item command="transfer" :disabled="single"
              v-hasPermi="['order:supplier:transfer']">转移供应商</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </el-col>
    ...
    <el-table border ref="brtTable" v-loading="loading" :data="supplierList" @selection-change="handleSelectionChange">
      ...
      <template v-for="(item, index) in cacheCloumnList">
        <el-table-column v-if="item.visible" :label="item.label" :prop="item.prop" :width="item.width || ''"
          ...
          <template slot="header" slot-scope="scope">
            {{ item.label }}
            <el-button v-if="item.prop == 'typeVo.typeName'" @click="handleOpenSupplierType" class="mt5 ml5"
              icon="el-icon-plus" size="mini" circle v-hasPermi="['order:supplierType:list']"></el-button>
            <el-button v-if="item.prop == 'gradeVo.gradeName'" @click="handleOpenSupplierGrade" class="mt5 ml5"
              icon="el-icon-plus" size="mini" circle v-hasPermi="['order:supplierGrade:list']"></el-button>
          </template>
          <template slot-scope="scope">
            ...
            <template v-else-if="item.type == 'other'">
              <el-button v-if="item.prop == 'supplierNo'" type="text"
                @click="hanldeToHome(scope.row)">{{ scope.row.supplierNo }}</el-button>
              <el-button v-if="item.prop == 'files'" type="text" @click="hanldeOpenFileList(scope.row)">查看</el-button>
            </template>
      ...
      <!-- <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
            v-hasPermi="['order:supplier:edit']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
            v-hasPermi="['order:supplier:remove']">删除</el-button>
        </template>
      </el-table-column> -->
    </el-table>
    ...
    <!-- 供应商类型弹框 -->
    <supplier-type ref="supplierTypeRef"></supplier-type>

    <!-- 供应商等级弹框 -->
    <supplier-grade ref="supplierGreadeRef"></supplier-grade>
    ...
    <!-- 文件列表 -->
    <file-list @updateFile="updateSupplierInfo" ref="fileListRef"></file-list>
...
export default {
  name: "Supplier",
  dicts: ["supplier_type", "supplier_payment_type", "supplier_grade"],
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      columnList: [
...

        {
          ...
          align: "center",
          type: "dict",
          dictType: "supplier_payment_type"
        },
        // {
        //   label: "付款人",
        //   prop: "payer",
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + "/order/supplier/importData"
      },
      // 转移参数
      transfer: {
        // 是否显示弹出层
  ...
  methods: {
    ...
    hanldeOpenFileList(row) {
      this.$refs.fileListRef.handleOpen(row, 'files')
    },
    /**
     * 提交转移
     */
    submitTransferForm() {
      updateSupplier(this.transfer.form).then(res => {
        this.$notify({
          title: '成功',
          message: '操作成功',
          type: 'success'
     ...
     * 打开供应商等级弹框
     */
    handleOpenSupplierGrade() {
      this.$refs.supplierGreadeRef.handleOpen();
    },
    /**
     * 打开供应商等级弹框
     */
    handleOpenSupplierType() {
      this.$refs.supplierTypeRef.handleOpen();
    },
    ...
    /** 下载模板操作 */
    importTemplate() {
      this.download('order/supplier/importTemplate', {}, `template_${new Date().getTime()}.xlsx`)
    },
    /** 查询供应商信息列表 */
    getList() {
      this.loading = true;
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('order/supplier/export', {
        ...this.queryParams,
        ids: this.ids
