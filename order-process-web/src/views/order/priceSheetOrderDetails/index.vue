<template>
  <div>
    <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <!-- <el-row>
          <el-col :span="12">
            <el-form-item label="boom单" prop="boomFile">
              <file-upload-button class="mr10" v-model="form.boomFile" :limit="1"></file-upload-button>
      ...
      <div v-for="(item,index) in priceSheetOrderDetailsList" class="order-details-list">
        <el-row>
          <el-col :span="23">
    ...
    <template v-if="!isEdit" class="priceSheetInfo">
    ...
    <el-divider></el-divider>
      <el-row>
        <el-col :span="24">
          <el-table :data="priceSheetData" style="width: 100%">
            <el-table-column prop="orderNo" label="报价单号"></el-table-column>
            <el-table-column prop="createTime" label="报价日期"></el-table-column>
          </el-table>
        </el-col>
      </el-row>
    </template>
...
<script>
  ...
  import {
    listPriceSheetOrderDetails,
    listPriceSheetOrderDetailsAll,
    getPriceSheetOrderDetails,
    delPriceSheetOrderDetails,
    addPriceSheetOrderDetails,
    updatePriceSheetOrderDetails
  ...
  export default {
    ...
    methods: {
      ...
      getPriceSheetrecordData(){
        let data = {
          orderId: this.orderId
        }
        listPriceSheetOrderRecordAll(data).then(res=>{
            this.priceSheetData = res.data
        })
      },
      checkRole,
      /**
       ...
       */
      totalOrderAmount() {
        let totalNum = this.priceSheetOrderDetailsList.reduce((total, item) => total + Number(item.detailsNum || 0), 0);
        let totalAmount = this.priceSheetOrderDetailsList.reduce((total, item) => total + Number(item.detailsAmount || 0),
          0);
      ...
      handleEdit(index) {
        ...
        this.priceSheetOrderDetailsList.splice(index, 1)
       ...
       */
      handleCopy(index) {
        let orderDetails = this.priceSheetOrderDetailsList[index];
       ...
       * 新增产品
       */
      async handleAddPriceSheetOrderDetails() {
       ...
       * 保存产品信息
       */
      async priceSheetOrderDetailsSave() {
        ...
        this.$refs["form"].validate(valid => {
          if (valid) {
            ...
            this.priceSheetOrderDetailsList.push(item);
            // 重置表单
            this.$nextTick(() => {
              this.reset();
            })
      ...
      getList() {
        this.loading = true;
        if (!this.orderId) {
          this.priceSheetOrderDetailsList = [];
          this.loading = false;
          return
        }
        this.queryParams.orderId = this.orderId;
        listPriceSheetOrderDetailsAll(this.queryParams).then(response => {
          this.priceSheetOrderDetailsList = response.data;
          this.loading = false;
        });
      },
      // 取消按钮
      ...
      //   getPriceSheetOrderDetails(detailsId).then(response => {
      ...
      handleDelete(row, index) {
        ...
        if (detailsIds) {
          this.$modal.confirm('确认删除？').then(function() {
            return delPriceSheetOrderDetails(detailsIds);
          }).then(() => {
            this.getList();
      ...
      /** 导出按钮操作 */
      handleExport() {
        this.download('order/priceSheetOrderDetails/export', {
          ...this.queryParams
        }, `priceSheetOrderDetails_${new Date().getTime()}.xlsx`)
      },
      setPriceSheetOrderDetailsList(sodl){
        this.priceSheetOrderDetailsList = sodl;
      },
      isShow(){
